    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
            ...
            if (ui.notificationsEnabled) {
                Text(
                    if (scheduler.exactAlarmReady) "5分前の正確なアラーム：有効" else "正確なアラーム：未許可（端末都合で通知が遅れる場合があります）",
                    style = MaterialTheme.typography.bodySmall,
                    color = if (scheduler.exactAlarmReady) MaterialTheme.colorScheme.primary else MaterialTheme.colorScheme.error
                )
            }
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                if (ui.notificationsEnabled && !scheduler.exactAlarmReady) {
                    OutlinedButton(onClick = {
                        scheduler.requestExactAlarmPermissionIntent()?.let(context::startActivity)
                    }) {
                        Text("5分前通知を正確にする")
                    }
                }
