...
for old, new in replacements:
    if text.count(old) != 1:
        raise SystemExit(f"Models.kt anchor mismatch: {old[:60]!r}")
...
upsert_start = text.index("    fun upsertEvaluatedRace(")
upsert_end = text.index("    fun settle(", upsert_start)
upsert = """    fun upsertEvaluatedRace(
        race: RaceData,
...
"""
text = text[:upsert_start] + upsert + text[upsert_end:]
