...
def build_records_from_kfiles(
    *,
    ...
    """Research copy with explicit end date and a hard 2025-Q4 fence."""
    start = date(year, 1, 1)
    end = end_date or date(year, 9, 30)
    if end.year != year or end < start:
        raise ValueError('invalid record period')
    if year >= 2025 and end > date(2025, 9, 30):
        raise ValueError('Q4 2025 holdout is sealed')
