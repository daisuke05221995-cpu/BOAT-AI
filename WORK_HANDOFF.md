...
### 2026-09-23 Work再開チェックポイント（最新）

- 安定公開版: v0.14.1。Value Strategyのassetは未公開、v0.15.0は未Release。
- 独立検証Run: **35814229078**。2021〜2024のKファイル学習4ジョブ成功、2023/2024/2025年別検証は実行中。結果ファイルはまだ存在しない。
- 最新Android Build Run: **35815647539**（commit 2279176536a8d8a67f01c12aba3b3300fb2fc64b、確認時in_progress）。古い失敗Run 35814862551 のKotlin公開範囲不一致は a0dace9 で修正し、Run 35815234144 はUnit test・lint・Debug APK生成まで成功。
- 新規作業: Value StrategyのBUY/SKIP・120通り・1〜10点・1200円配分のUnit test、一覧/一括購入見積もりと確定購入買い目の統一、買い目別の仮想投資/払戻保存、モデル次元不一致時の安全見送り。
- **次の1手:** Run 35815647539 のjob/stepを確認し、失敗ならログを見て修正。続いて独立検証Run 35814229078 の3年結果と data/multiyear_validation.json を確認。releaseCandidate=true の場合に限りPython/Android parity、Exporter、Release工程を続行。falseなら厳格条件を緩めずReleaseしない。
