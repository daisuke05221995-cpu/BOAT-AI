...
Work側のCORE r1研究1巡と2024年通年評価は完了。本番反映は行っていない。
...
- 2024通年cache: 52,179レース、対象確定済み6艇53,225の98.03%を収録
...
- 十分な購入件数を持つ最高ROI: **94.081871%**
  ...
  - 払戻 1,930,560円
  - 損益 -121,440円
- 最良条件: blend0.35 / minEV1.0 / minProbability0.01 / maxOdds20 / maxPoints3 / budget1200
- 市場補正系の最高ROIは77.73%、縮約補正は購入件数不足
- 5〜9月限定screenで見えた108.21% / 235.33%の候補は購入28 / 15件・各年1的中・払戻1件依存で不採用
- 2025 Q4 final holdoutは未開封
- 適格COREを固定できなかったためLONGSHOT BUY / CORE+LONGSHOT+SKIPの3分類検証は未着手
- `data/v016_annual2024_result.json` / `data/v016_core_research_report.md` / `data/v016_research_status.json` に研究結果を保存済み
- 同じ144条件グリッドは再実行せず不採用として固定する

### v0.16研究の次の方針

- 過去に見たROIへ閾値を合わせるだけの探索を繰り返さない
- 確率補正・閾値調整中心ではなく、新しい事前特徴量と取得時刻監査を伴うCORE仮説を事前登録してから検証する
- 2024通年cacheはRun `35870380669` のArtifact `v016-annual-cache-2024` を再利用する
- 新しいCOREが年間105%以上で固定できた後にのみLONGSHOTを開始する
- CORE / LONGSHOT双方を完全固定した後にのみ2025 Q4 final holdoutを一度だけ開く
- 通常チャット側の確認前に研究モデルを本番へ昇格しない
...
10. v0.16は同じ144条件を再実行せず、新CORE仮説の設計から再開
